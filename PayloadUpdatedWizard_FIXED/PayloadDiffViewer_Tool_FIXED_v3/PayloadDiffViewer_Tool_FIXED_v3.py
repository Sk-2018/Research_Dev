...
def _excel_headers_rows(path: str, sheet: Optional[str] = None, progress_cb: Optional[callable] = None) -> Tuple[List[str], List[List[str]]]:
    """
    Optimized Excel loader.

    1) Try UltraFastLoader (if available) for very fast reads and progress.
    2) Fallback to pandas.read_excel with header scoring (previous behavior).
    """
    if not HAS_PANDAS:
        raise RuntimeError("pandas is required to read Excel files")

    # 1) UltraFastLoader path
    if globals().get("HAS_ULTRA_LOADER", False) and "ExternalUltraFastLoader" in globals():
        try:
            logger.info("Using UltraFastLoader for Excel file: %s", path)
            loader = ExternalUltraFastLoader()
            ...
            def ultra_progress(current, total):
                ...
                progress_cb(
                    min(100, max(0, pct)),
                    f"Read ~{current:,} rows (Ultra Excel)"
                )

            # For Excel we typically don't chunk by a fixed row size here;
            # UltraFastLoader will decide the best strategy.
            for chunk in loader.load_chunked(
                path,
                chunk_size=None,
                progress_callback=ultra_progress,
            ...
            logger.warning("UltraFastLoader Excel path failed, falling back to pandas: %s", e)
            if progress_cb:
                progress_cb(0, f"UltraFastLoader Excel failed ({e}), using pandas reader...")

    # 2) Fallback: original pandas implementation
    try:
...
def assemble_rows(headers: List[str], raw_rows: List[List[str]], mapping: Dict[str, int]) -> List[Dict[str, str]]:
    ...
    if HAS_PANDAS:
        try:
            ...
            # Columns map from index back to role name
            df_subset.columns = [idx_to_role[i] for i in valid_indices]
            ...
            logger.warning(f"Pandas assembly failed ({e}), falling back to Python loop.")

    # Fallback: simple Python loop, lower peak memory but slower
    idxs = [(role, col_indices[role]) for role in NEEDED_ROLES if col_indices[role] != -1]
    rows: List[Dict[str, str]] = []
    for raw in raw_rows:
        row = {}
        for role, i in idxs:
            row[role] = raw[i] if 0 <= i < len(raw) else ""
        rows.append(row)
    return rows
...
class PayloadDiffViewerApp(tk.Tk):
    def __init__(self):
        ...
        # Map Config Name -> list of integer indices into self.rows (memory friendly)
        ...
        self.rows_to_compare_map: Dict[str, Dict[str, str]] = {}
    ...
    def _build_ui(self):
        ...
        # Enable copy-from-row on the top pane (Treeview)
        ...
        # Make inline and JSON panes read-only but copyable
        self._make_text_readonly_copy(self.txt_sel_old)
        self._make_text_readonly_copy(self.txt_sel_new)
        self._make_text_readonly_copy(self.txt_old)
        self._make_text_readonly_copy(self.txt_cur)


    def _make_text_readonly_copy(self, widget: tk.Text):
        """
        Make a Text widget behave like a read-only viewer: content cannot be
        changed by the user, but they can still select and copy text out.
        """
        ...
        # Right-click menu with Copy / Select All only
        ...
        # Allow Ctrl+A / Ctrl+C, but block any edits (typing, delete, paste)
        ...
        def on_key(event):
            ...
            if event.keysym in ("Left", "Right", "Up", "Down",
                                "Home", "End", "Prior", "Next"):
                return
            # Allow copy/select-all combos (handled above)
            if (event.state & 0x4) and event.keysym in ("c", "C", "a", "A"):
                return
            # Block everything else that could change content
            if event.char:
                return "break"
            return
    ...
    def _setup_tree_copy_paste(self):
        """
        Add a simple 'Copy row' capability to the main Treeview so that the
        selected row can be copied and pasted into other tools.
        """
        tree = self.tree
        menu = tk.Menu(tree, tearoff=False)

        def do_copy_row(event=None):
            item = tree.focus()
            if not item:
                sel = tree.selection()
                if not sel:
        ...
        menu.add_command(label="Copy row", command=do_copy_row)

        def popup(event):
            # Focus the row under the cursor before showing the menu
        ...
        tree.bind("<Control-c>", do_copy_row)
        tree.bind("<Control-C>", do_copy_row)
    ...
    def on_open(self):
        ...
        use_chunked = HAS_PANDAS and (ext in ('.csv', '.tsv', '.txt') or (ext in ('.xlsx', '.xls') and HAS_ULTRA_LOADER))
        ...
        def load_task(progress_cb=None):
            if ext in ('.csv', '.tsv', '.txt'):
                ...
                try:
                    return _excel_headers_rows(p, progress_cb=progress_cb)
                except Exception as e:
                    raise
    ...
    def _finalize_load(self):
        ...
        # Build an index from Config Name -> list of row indices
        ...
        self.lbl.configure(
            text=(
                ...
                f"{len(invalid_names_found)} invalid Config Name(s) skipped. "
                "Select a Config Name."
            )
        )

        self._reset_views()
        logger.info(
            "File loaded. %d valid config names found. %d invalid names skipped.",
            len(self.by_name),
            len(invalid_names_found),
        )

        # Encourage Python to free any large temporary objects early
        gc.collect()
    ...
    def on_name_selected(self, _evt=None):
        ...
        # Look up row indices for this Config Name
        indices = self.by_name.get(n, [])
        # Build display keys and keep a display->raw mapping for matching later
        self.display_to_raw = {}
        disp_keys = set()
        for i in indices:
            raw = (self.rows[i].get('Config Key', '') or '').strip()
            if not raw:
                continue
            disp = self._format_key(raw)
            if disp not in self.display_to_raw:
                self.display_to_raw[disp] = raw
            disp_keys.add(disp)
        keys = sorted(disp_keys)
    ...
    def _get_rows_for_keys_map(self, name: str, keys: List[str]) -> Dict[str, Dict[str, str]]:
        '''
        Build a map of {cfgkey_display: row_dict} for comparison.

        The Listbox shows formatted/display keys. We map those back to the raw
        values from the file for matching, but keep the display key so the UI
        and exports stay consistent.
        '''
        disp_set = set(keys)
        raw_to_disp: Dict[str, str] = {}
        for disp in disp_set:
            raw = (self.display_to_raw.get(disp, disp) or '').strip()
            if raw:
                raw_to_disp[raw] = disp
        remaining_raw = set(raw_to_disp.keys())

        rows_map: Dict[str, Dict[str, str]] = {}
        for idx in self.by_name.get(name, []):
            row = self.rows[idx]
            raw_k = (row.get('Config Key', '') or '').strip()
            if raw_k in remaining_raw:
                disp_k = raw_to_disp.get(raw_k, raw_k)
                rows_map[disp_k] = row
                remaining_raw.remove(raw_k)
                if not remaining_raw:
                    break

        return rows_map

    def on_compare(self, _evt: Optional[tk.Event] = None) -> None:
        """
        Entry point for the Compare button and F5 shortcut.

        1. Validates that a Config Name and one or more Config Keys are selected.
        2. Builds a compact map of rows to compare (using indices from self.by_name).
        3. Runs DeepDiff in a background thread pool with a progress dialog.
        4. Updates the main table + stats when done.
        """
        name = self.cmb_name.get().strip()
        if not name:
            messagebox.showwarning('Config Name', 'Please select a Config Name first.')
            return

        keys = self._get_selected_config_keys()
        if not keys:
            return

        rows_map = self._get_rows_for_keys_map(name, keys)
        if not rows_map:
            messagebox.showwarning('No Rows', 'No matching rows found for the selected Config Keys.')
            return

        # Prepare for comparison
        self.rows_to_compare_map = rows_map
        ...
        def done_cb(result: Tuple[List[RowMeta], Dict[str, int]]):
            # Re-enable Compare button even if something goes wrong
            self.btn_compare.configure(state='normal')
            ...
            if total_diffs == 0:
                messagebox.showinfo('No Differences', 'DeepDiff found no differences for the selected keys.')
                self._reset_views(clear_keys=False)
                return

            if total_diffs > config.DIFF_DISPLAY_LIMIT:
                if messagebox.askyesno(
                    'Limit Results',
                    f'{total_diffs:,} differences found. Show only the first {config.DIFF_DISPLAY_LIMIT:,} rows?'
                ):
                    diffs = diffs[:config.DIFF_DISPLAY_LIMIT]

            # Sort and display
            diffs.sort(key=lambda m: (m.cfgkey, m.path or ''))
            ...
            # Enable export & clear now that we have data
            self.btn_clear.configure(state='normal')
            self.btn_export_csv.configure(state='normal')
            self.btn_export_txt.configure(state='normal')

        # Launch background work with a progress dialog
        self._with_progress_threaded(
            self._run_parallel_diffs,
            title=f"Comparing {len(rows_map):,} Config Key(s)...",
            done_cb=done_cb,
            determinate=True,
        )
    ...
    def _diff_worker(self, tasks_q: queue.Queue, results_q: queue.Queue, parse_log_q: queue.Queue, ignore_order: bool):
        ...
        """
        # --- Missing-payload business rules (diff-only) ---
        def _is_missing_payload(v) -> bool:
            """Treat empty/NaN/None/'nan'/'null' as missing payload."""
            if v is None:
                return True
            # NaN check (works for float/numpy float)
            try:
        ...
        def _iter_leaf_paths(obj, prefix: str = ""):
            """Yield (path, value) for leaf nodes using 'a.b[0].c' paths."""
            if obj is None:
                return
            if isinstance(obj, dict):
                for k, v in obj.items():
                    k_str = str(k)
                    p2 = f"{prefix}.{k_str}" if prefix else k_str
                    yield from _iter_leaf_paths(v, p2)
        ...
        while True:
            try:
                ...
                # --- Business rule: classify missing payloads as added/removed/skip ---
                old_missing = _is_missing_payload(old_str)
                cur_missing = _is_missing_payload(new_str)

                # If both missing -> skip comparison (no diffs)
                if old_missing and cur_missing:
                    results_q.put((cfgkey, (None, None), []))
                    continue

                # OLD missing -> everything in CURRENT is 'added'
                if old_missing and not cur_missing:
                    cur_obj, err2 = parse_jsonish_verbose(new_str)
                    if err2 and err2 != 'Empty payload':
                        parse_log_q.put((f"[{cfgkey}] CURRENT: {err2}", 'warning', str(new_str)[:200]))
                    diff_list = []
                    if cur_obj is None and str(new_str).strip():
                        # If parsing failed, treat raw as a single added root value
                        diff_list.append(RowMeta(cfgkey, 'added', '', None, str(new_str)))
                    else:
                        for kpath, val in _iter_leaf_paths(cur_obj):
                            diff_list.append(RowMeta(cfgkey, 'added', kpath, None, val))
                    results_q.put((cfgkey, (None, cur_obj), diff_list))
                    continue

                # CURRENT missing -> everything in OLD is 'removed'
                if cur_missing and not old_missing:
                    old_obj, err1 = parse_jsonish_verbose(old_str)
                    if err1 and err1 != 'Empty payload':
                        parse_log_q.put((f"[{cfgkey}] OLD: {err1}", 'warning', str(old_str)[:200]))
                    diff_list = []
                    if old_obj is None and str(old_str).strip():
                        diff_list.append(RowMeta(cfgkey, 'removed', '', str(old_str), None))
                    else:
                        for kpath, val in _iter_leaf_paths(old_obj):
                            diff_list.append(RowMeta(cfgkey, 'removed', kpath, val, None))
                    results_q.put((cfgkey, (old_obj, None), diff_list))
                    continue
    ...
    def _format_key(self, k: str) -> str:
        """Format scientific notation to a non-exponent string for display (precision-safe)."""
        s = str(k).strip()
        if not s:
            return s
        try:
            d = Decimal(s)
            out = format(d, 'f')  # no exponent
            if '.' in out:
    ...
    def on_view_summary(self):
        ...
        # New: controls for Grand Total display
        win.total_scope = tk.StringVar(value='All')  # Options: All, Visible, Selected
        win.show_total = tk.BooleanVar(value=True)
        win.grand_total_var = tk.StringVar(value='')
        # New: config-name filter combobox (single) and multiselect storage
        win.cfg_name_multiset = []
        ...
        # Search removed; use Multi-select to filter instead
        # Config selection is handled via the Multi-select dialog (button below)
        def open_multi_select_dialog():
            dlg = tk.Toplevel(win)
            dlg.title('Select Config Names')
            dlg.transient(win)
            dlg.grab_set()
            ...
            lbl = ttk.Label(frame, text='Select one or more Config Names:')
            lbl.pack(anchor='w')
            ...
            # Populate listbox from current pivot data
            names = [n for n, _ in (win.pivot_data or [])]
            for nm in names:
                lb.insert(tk.END, nm)

            # Pre-select any previously chosen names
            if win.cfg_name_multiset:
                for i, nm in enumerate(names):
                    if nm in win.cfg_name_multiset:
                        lb.select_set(i)

            ctrl_frame = ttk.Frame(frame)
            ctrl_frame.pack(fill=tk.X, pady=(6,0))

            ttk.Label(ctrl_frame, text='Random N:').pack(side=tk.LEFT)
            spn = ttk.Spinbox(ctrl_frame, from_=1, to=max(1, len(names)), width=5)
            spn.set('1')
            spn.pack(side=tk.LEFT, padx=(6, 8))
            ...
            def do_random():
                ...
                chosen = random.sample(names, n) if names else []
            ...
            ttk.Button(ctrl_frame, text='Random', command=do_random).pack(side=tk.LEFT, padx=(0,6))

            btn_frame = ttk.Frame(frame)
            btn_frame.pack(fill=tk.X, pady=(8,0))
            def on_ok():
                sel = [lb.get(i) for i in lb.curselection()]
                win.cfg_name_multiset = sel
                dlg.destroy()
                update_view()
            ...
            ttk.Button(btn_frame, text='OK', command=on_ok).pack(side=tk.RIGHT, padx=6)
            ttk.Button(btn_frame, text='Clear', command=on_clear).pack(side=tk.RIGHT)

        ttk.Button(ctrl, text='Multi-select...', command=open_multi_select_dialog).pack(side=tk.LEFT, padx=(2,6))
        ...
        # Top-N removed; show all selected items
        ...
        # Grand total scope & toggle
        ttk.Label(ctrl, text='Grand total:').pack(side=tk.RIGHT, padx=(8, 2))
        ttk.Label(ctrl, textvariable=win.grand_total_var).pack(side=tk.RIGHT, padx=(2, 6))
        cmb_scope = ttk.Combobox(ctrl, values=['All', 'Visible', 'Selected'], textvariable=win.total_scope, width=10, state='readonly')
        cmb_scope.pack(side=tk.RIGHT, padx=(2, 6))
        cmb_scope.bind('<<ComboboxSelected>>', lambda e: update_view())
        ttk.Checkbutton(ctrl, text='Show Grand Total', variable=win.show_total, command=lambda: update_view()).pack(side=tk.RIGHT, padx=(6, 0))

        # Use horizontal split: left pane for Config names, right pane for Chart
        body = ttk.PanedWindow(win, orient=tk.HORIZONTAL)
        body.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))

        tbl_frame = ttk.Frame(body)
        # give the table pane a smaller weight than the chart pane
        body.add(tbl_frame, weight=1)
        ...
        # narrower width for the left pane when using side-by-side layout
        tree.column('Config Name', width=360, anchor='w')
        ...
        # Reserve a row for the totals label below the table, move horizontal scrollbar below that
        tbl_frame.rowconfigure(0, weight=1)
        tbl_frame.columnconfigure(0, weight=1)
        totals_frame = ttk.Frame(tbl_frame)
        totals_frame.grid(row=1, column=0, columnspan=2, sticky='ew')
        hs.grid(row=2, column=0, sticky='ew')
        tbl_frame.rowconfigure(2, weight=0)
        ...
        # Bottom/table grand total label placed under the table
        try:
            ttk.Label(totals_frame, textvariable=win.grand_total_var).pack(anchor='w', padx=6, pady=(6, 4))
        except Exception:
            pass
        ...
        def apply_filters(data: List[Tuple[str, int]]) -> List[Tuple[str, int]]:
            ...
            # 3) Config-name multiselect filter (from dialog)
            if getattr(win, 'cfg_name_multiset', None):
                selset = {n.lower() for n in win.cfg_name_multiset}
                data = [d for d in data if d[0].lower() in selset]
            # 4) No Top-N filtering (removed)
        ...
        def draw_chart(data: List[Tuple[str, int]]):
            ...
            # Vertical bar chart
            width = max(6.0, min(30.0, 0.40 * ncat + 3.0))
            fig = Figure(figsize=(width, 6), dpi=100)
            ...
            if win.show_values.get():
                ...
                for i, v in enumerate(counts):
                    ax.text(i, v + offset, str(v), ha='center', va='bottom', fontsize=max(8, fs-1))
        ...
        def update_view(*_):
            ...
            pivot_map = {name: cnt for name, cnt in win.pivot_data} if win.pivot_data else {}

            # If the user has an active multiselect, always use that as the source
            # of truth for totals (this is the requested behavior).
            if getattr(win, 'cfg_name_multiset', None):
                mult = list(win.cfg_name_multiset)
                total_cfg = len(mult)
                total_rows = sum(pivot_map.get(n, 0) for n in mult)
            else:
                # Compute totals according to scope selection when no multiselect
                scope = (win.total_scope.get() or 'All').lower()
                if scope == 'visible':
                    total_cfg = len(data)
                    total_rows = sum(cnt for _, cnt in data) if data else 0
                elif scope == 'selected':
                    sel = tree.selection()
                    selected_names = [tree.item(i, 'values')[0] for i in sel] if sel else []
                    total_cfg = len(selected_names)
                    total_rows = sum(pivot_map.get(n, 0) for n in selected_names)
                else:  # 'all' or default
                    total_cfg = len(win.pivot_data)
                    total_rows = sum(cnt for _, cnt in win.pivot_data) if win.pivot_data else 0
            # Update toolbar and bottom grand total display
            try:
                if win.show_total.get():
                    win.grand_total_var.set(f"{total_cfg} configs, {total_rows:,} rows")
                else:
                    win.grand_total_var.set("")
            except Exception:
                pass
            ...
            try:
                ...
                # status may also have been removed; ignore if it's gone
                pass

        # search box removed; no trace needed
        ...
        def after_pivot_ready(data: List[Tuple[str, int]]):
            win.pivot_data = data
            # Populate any selection UI if needed (multiselect dialog reads pivot_data)
            update_view()
