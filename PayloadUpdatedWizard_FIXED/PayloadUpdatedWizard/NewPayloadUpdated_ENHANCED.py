...
def save_xlsx(df, path_xlsx):
    from pandas import ExcelWriter
    try:
        with ExcelWriter(path_xlsx, engine="openpyxl", mode="w") as xw:
            df.to_excel(xw, index=False, sheet_name="export")
    except PermissionError as e:
        raise RuntimeError(f"Cannot write {path_xlsx}. If it’s open in Excel, close it and retry.") from e
...
# ------------------------ SQL guards ------------------------
FORBIDDEN = re.compile(r"(INSERT|UPDATE|DELETE|DROP|ALTER|TRUNCATE|MERGE|CREATE|GRANT|REVOKE|VACUUM|ANALYZE|EXPLAIN|CALL|DO)", re.I)
...
def connect_pg(host: str, port: int, dbname: str, user: str, pwd: str, *, sslmode=None, sslrootcert=None, app_name="payload-wizard"):
    options = f"-c statement_timeout={STATEMENT_TIMEOUT_MS} -c idle_in_transaction_session_timeout={IDLE_SESSION_TIMEOUT_MS} -c default_transaction_read_only=on -c application_name={app_name}"
    last = None
    for wait in CONNECT_RETRY_DELAYS:
        ...
        try:
            if HAVE_PG3:
                kwargs = dict(host=host, port=port, dbname=dbname, user=user, password=pwd,
                              connect_timeout=CONNECTION_TIMEOUT_SEC, options=options)
                if sslmode: kwargs["sslmode"] = sslmode
                if sslrootcert: kwargs["sslrootcert"] = sslrootcert
                conn = psycopg3.connect(**kwargs)
            else:
                kwargs = dict(host=host, port=port, dbname=dbname, user=user, password=pwd,
                              connect_timeout=CONNECTION_TIMEOUT_SEC, options=options)
                if sslmode: kwargs["sslmode"] = sslmode
                ...
                try:
                    ...
                    pass
            # Optional: set app name again for visibility
            try:
                with conn.cursor() as cur:
...
class PayloadComparisonFrame(ttk.Frame):
    def __init__(self, master, app):
        ...
        self.app = app
        self.running = False
        if not hasattr(self.app, "cancel_event"):
        ...
        self.btn_cancel = ttk.Button(self, text="Cancel", command=self.on_cancel, state=tk.DISABLED)
        self.btn_cancel.grid(row=3, column=6, sticky="e", **pad)
    ...
    def _worker(self, cfg, user, pwd, sql, outfile_csv: Path, outfile_xlsx: Path, use_copy: bool, outdir: str):
        ...
        try:
            ...
            if use_copy:
                ...
                sql = enforce_limit(sql)
                self.log("Cursor fetch path (server-side streaming)")
                cur = conn.cursor()
                try:
