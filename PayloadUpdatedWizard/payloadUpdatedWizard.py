# -*- coding: utf-8 -*-
"""
payload_wizard_v122.py

Enhanced version with auto-disconnect + auto-launch workflow:
- DB connection closes IMMEDIATELY after export
- Shows friendly message: "Files downloaded, starting comparison..."
- Auto-launches PayloadDiffViewer (GeminiPayloadDiff.py) with exported file
- All SELECT-only guardrails preserved
...
def auto_fix_sql(sql: str, schema: str):
    ...
    if "{schema}" in sql:
        ...
        notes.append(f"filled {{schema}} → {schema}")
    if schema and schema != "sdc_owner":
        newsql = re.sub(r"\bsdc_owner\.", f"{schema}.", sql)
        if newsql != sql:
            sql = newsql
            notes.append(f"rewrote sdc_owner.* → {schema}.*")
...
class CredsScreen(ttk.Frame):
    def __init__(self, master, app):
        ...
        self.cb_host = ttk.Combobox(
            self, textvariable=self.var_host, values=[], state="readonly", width=42
        )
        self.cb_host.grid(row=0, column=1, sticky="we", **pad)
        ttk.Label(self, text="Port").grid(row=0, column=2, sticky="w", **pad)
        ttk.Entry(self, textvariable=self.var_port, width=10).grid(
            row=0, column=3, sticky="w", **pad
        )
        ttk.Label(self, text="DB").grid(row=0, column=4, sticky="w", **pad)
        ttk.Entry(self, textvariable=self.var_db, width=18).grid(
    ...
    def on_next(self):
        ...
        cfg["host"] = host
        self.app.selection.update(
            {"host": host, "port": port, "db": db, "user": user, "pwd": pwd, "outdir": outdir, "cfg": cfg}
        )
...
class UtilityScreen(ttk.Frame):
    def __init__(self, master, app):
        ...
        ttk.Button(self, text="← Back", command=self.app.show_creds).grid(
...
class PayloadComparisonFrame(ttk.Frame):
    def __init__(self, master, app):
        ...
        ttk.Label(
            self, text=f"Driver detected: {drv} | OS: {'Windows' if os.name=='nt' else os.name}"
        ...
        ttk.Radiobutton(
            self, text="Use Default Template", variable=self.var_mode, value="default"
        ).grid(row=1, column=1, sticky="w", **pad)
        ttk.Radiobutton(
            self, text="Use Custom SQL", variable=self.var_mode, value="custom"
        ...
        ttk.Checkbutton(
            self, text="Use COPY mode (keeps timestamps as text)", variable=self.var_copy
        ).grid(row=4, column=0, sticky="w", **pad)

        self.var_autofix = tk.IntVar(value=1)
        ttk.Checkbutton(
            self,
            text="Auto-fix SQL (fill {schema}, trim to first SELECT, remove trailing ';')",
            variable=self.var_autofix,
        ).grid(row=4, column=1, sticky="w", **pad)

        ttk.Label(self, text="Viewer Path (GeminiPayloadDiff.py):").grid(
            row=4, column=2, sticky="w", **pad
        )
        self.e_viewer = ttk.Entry(self, width=40)
        self.e_viewer.insert(0, os.environ.get("PAYLOADDIFF_VIEWER_PATH", ""))
        self.e_viewer.grid(row=4, column=3, columnspan=3, sticky="we", **pad)
        ttk.Button(self, text="Find Viewer", command=self.on_pick_viewer).grid(
            row=4, column=6, sticky="e", **pad
        )

        ttk.Button(self, text="Load SQL", command=self.on_load_sql).grid(
            row=5, column=0, sticky="w", **pad
        )
        ttk.Button(self, text="Save SQL", command=self.on_save_sql).grid(
            row=5, column=1, sticky="w", **pad
        )
        ttk.Button(self, text="Reset to Default", command=self.on_reset_default).grid(
            row=5, column=2, sticky="w", **pad
        )

        self.btn_run = ttk.Button(
            self, text="Run → Export & Launch Viewer", command=self.on_run
        )
        ...
        self.lbl_audit = ttk.Label(
            self, text="Audit will be created in your output folder"
        )
    ...
    def on_pick_viewer(self):
        p = filedialog.askopenfilename(
            title="Select GeminiPayloadDiff.py (PayloadDiffViewer)",
    ...
    def on_load_sql(self):
        p = filedialog.askopenfilename(
            title="Load SQL", filetypes=[("SQL", "*.sql"), ("Text", "*.txt"), ("All", "*.*")]
        )
        if not p:
            return
        try:
    ...
    def on_save_sql(self):
        p = filedialog.asksaveasfilename(
            title="Save SQL", defaultextension=".sql", filetypes=[("SQL", "*.sql"), ("Text", "*.txt")]
        )
        if not p:
            return
        try:
    ...
    def on_run(self):
        ...
        if mode == "custom" and bool(self.var_autofix.get()):
            ...
            if fixed != sql:
                ...
                for n in notes:
                    self.log(f"auto-fix: {n}")
            sql = fixed

        if not looks_like_select_only(sql):
            messagebox.showerror(
                "Only SELECT allowed",
                "Provide a single SELECT or WITH ... SELECT statement.",
            )
            return
        ...
        t = threading.Thread(
            target=self._worker,
            args=(cfg, user, pwd, sql.strip(), outfile, use_copy, viewer_path, outdir),
            daemon=True,
        )
        t.start()

    def _worker(self, cfg, user, pwd, sql, outfile, use_copy, viewer_path, outdir):
        ...
        try:
            ...
            if use_copy:
                ...
                fd, tmpcsv = tempfile.mkstemp(prefix="pg_export_", suffix=".csv")
                os.close(fd)
                cur = conn.cursor()
                try:
                    if HAVE_PG3:
            ...
            # ─────────────────────────────────────────────────────────────
            # CRITICAL: Close DB connection BEFORE writing Excel
            # ─────────────────────────────────────────────────────────────
            if conn is not None:
                try:
                    conn.close()
                    ...
                    self.log(f"Warning: Failed to close connection: {e}")

            # Write Excel after disconnect
            meta = pd.DataFrame(
                {
                    "key": [
                        "app_version",
                    ...
                    "value": [
                        APP_VERSION,
                        "Windows" if os.name == "nt" else os.name,
            ...
            # ─────────────────────────────────────────────────────────────
            # Show friendly message BEFORE launching viewer
            # ─────────────────────────────────────────────────────────────
            self.after(
                0,
                messagebox.showinfo,
                "Download Complete",
                f"Files downloaded successfully ({rowcount} rows).\n\n"
                f"Database connection closed.\n\n"
                f"Starting comparison tool...",
            )
            ...
            if viewer_path and os.path.isfile(viewer_path):
                ...
                try:
                    ...
                    self.log("✅ Viewer launched successfully.")
                except Exception as e:
                    self.log(f"⚠️ Viewer launch warning: {e}")
            else:
                self.log("⚠️ Viewer path invalid; skipping viewer launch.")
            ...
            if conn is not None:
                try:
                    conn.close()
                    self.log("Database connection closed (fallback cleanup).")
                except Exception:
                    pass
            time.sleep(0.05)
            ...
            q_hash = hashlib.sha256(sql_log.encode("utf-8", errors="ignore")).hexdigest()
