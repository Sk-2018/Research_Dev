...
def normalize_for_diff(x: Any, *, coerce_numeric: bool = True) -> Any:
    if isinstance(x, dict): return {k: normalize_for_diff(v, coerce_numeric=coerce_numeric) for k,v in x.items()}
    if isinstance(x, list): return [normalize_for_diff(v, coerce_numeric=coerce_numeric) for v in x]
...
def run_deepdiff(old: Any, new: Any, *, ignore_order: bool, coerce_numeric: bool = True) -> dict:
    ...
    dd = DeepDiff(norm_old, norm_new, ignore_order=ignore_order, report_repetition=True, verbose_level=2, view="text")
    return dd.to_dict()
