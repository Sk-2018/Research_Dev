...
"""File loader with delimiter sniffing, chunked CSV reading, and Excel (.xlsx) support."""
...
class FileLoader:
        ...
        with open(path, "r", encoding="utf-8", errors="ignore", newline="") as f:
            ...
            for i, row in enumerate(reader):
                if headers is None:
                    ...
                    continue
                rows.append(row)
                read_bytes += sum(len(c) for c in row) + len(row)  # crude
                if i % 5000 == 0:
    ...
    def load_excel(
        self, path: str, sheet: str | None = None, progress_cb: ProgressCB | None = None
