...
class SummaryDashboard(Toplevel):
    ...
    def _build(self) -> None:
        ...
        self._tree.heading("name", text="Config Name")
    ...
    def _pivot(self):
        ...
        gt = _pd.DataFrame({"Config Name":["Grand Total"], "Count":[total]})
        pivot = _pd.concat([pivot, gt], ignore_index=True)
    ...
    def _refresh_chart_debounced(self) -> None:
        ...
        self._debounce = _t.Timer(0.25, self._refresh_chart)
    ...
    def _save_png(self) -> None:
        ...
        fig = _Figure(figsize=(6,4), dpi=120)
        ax = fig.add_subplot(111)
        ax.barh(range(len(labels)), counts)
        ax.set_yticks(range(len(labels)))
        ax.set_yticklabels(labels)
