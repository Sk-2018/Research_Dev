...
class SummaryDashboard(Toplevel):
    ...
    def _build(self) -> None:
        ...
        self._tree.grid(row=0, column=0, sticky=N+S+E+W); vsb.grid(row=0, column=1, sticky=N+S)
        body.add(left, weight=1)
        right = ttk.Frame(body); right.grid_columnconfigure(0, weight=1); right.grid_rowconfigure(0, weight=1)
        self._chart_container = right; body.add(right, weight=1)
    ...
    def _pivot(self):
        ...
        pivot = (self._df[[col]].astype(str).rename(columns={col: "Config Name"})
                 .assign(**{"Config Name": lambda d: d["Config Name"].str.extract(r"([A-Za-z_]+)", expand=False)})
                 .dropna().value_counts().reset_index(name="Count"))
    ...
    def _refresh_table(self) -> None:
        ...
        pivot = self._pivot()
        if pivot is None:
            self._status.set("No data. Chart unavailable."); return
    ...
    def _draw_chart(self) -> None:
        ...
        if Figure is None:
            ttk.Label(self._chart_container, text="Chart unavailable. Install matplotlib for visualization.").pack(padx=12, pady=12); return
        pivot = self._pivot()
        if pivot is None or pivot.empty:
            ttk.Label(self._chart_container, text="No data to plot.").pack(padx=12, pady=12); return
        ...
        fig = Figure(figsize=(5.5,4.0), dpi=100); ax = fig.add_subplot(111)
        ax.barh(range(len(labels)), counts)
        ax.set_yticks(range(len(labels))); ax.set_yticklabels(labels)
        ax.invert_yaxis(); ax.set_xlabel("Count"); ax.set_title("Config Name Frequency")
        fig.tight_layout()
        canvas = FigureCanvasTkAgg(fig, master=self._chart_container); canvas.draw()
    ...
    def _save_png(self) -> None:
        ...
        fig = _Figure(figsize=(6,4), dpi=120); ax = fig.add_subplot(111)
        ax.barh(range(len(labels)), counts)
        ax.set_yticks(range(len(labels))); ax.set_yticklabels(labels)
        ax.invert_yaxis(); ax.set_xlabel("Count"); ax.set_title("Config Name Frequency")
        fig.tight_layout()
