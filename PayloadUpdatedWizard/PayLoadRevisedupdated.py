...
- View → Summary (Ctrl+M): Summary Dashboard
...
- File → Set Default Folder… : Persist a default starting folder for "Open…"
...
class PayloadDiffViewerApp(tk.Tk):
    ...
    def _get_initial_open_dir(self) -> Optional[str]:
        """Decide initial directory for File→Open dialogs."""
    ...
    def _set_default_folder(self):
        """File → Set Default Folder… action."""
    ...
    def on_view_summary(self):
        """Open the Summary Dashboard: pivot table + optional chart."""
        if not self.rows:
        ...
        tree.column('Config Name', width=520, anchor='w')
        ...
        # Chart area (below the table)
        chart_frame = ttk.Frame(body)
        body.add(chart_frame, weight=3)

        chart_info_lbl = None
        if not HAS_MPL:
            chart_info_lbl = ttk.Label(chart_frame, text="Chart unavailable. Install matplotlib for visualization.")
            chart_info_lbl.pack(pady=10)

        progress_lbl = ttk.Label(chart_frame, text="Generating summary…")
        progress_lbl.pack(anchor='w', padx=6, pady=(6, 0))

        # ---- Helper functions inside the Summary window scope ----
        ...
        def compute_pivot() -> List[Tuple[str, int]]:
            """Compute pivot: (Config Name, Count). Uses pandas if available else dict."""
            # Build data source with Config Name (skip empties)
            if pd is not None:
                try:
                    ...
                    ser = ser[ser != '']
                    pivot = ser.value_counts()  # already sorted by count desc
                    return [(idx, int(cnt)) for idx, cnt in pivot.items()]
                except Exception:
                    pass
        ...
        def shorten(label: str, maxlen: int = 24) -> str:
        ...
        def draw_chart(data: List[Tuple[str, int]]):
            ...
            for w in chart_frame.pack_slaves():
                if isinstance(w, ttk.Label) and w is not progress_lbl:
                    w.destroy()
            if win.canvas:
                try:
            ...
            if not data:
                lbl = ttk.Label(chart_frame, text="No data to chart.")
                lbl.pack(pady=10)
                return
            ...
            many = len(names) > 10
            orientation = 'horizontal' if many else 'vertical'
            ...
            if orientation == 'vertical':
                ...
                labels = [shorten(n, 22) for n in names]
                ax.set_xticks(list(ticks))
                ax.set_xticklabels(labels, rotation=45, ha='right')
                ax.set_ylabel("Count")
                ax.set_xlabel("Config Name")
            else:
                # Horizontal improves readability for many categories
                ax.barh(range(len(names)), counts)
                ticks = range(len(names))
                labels = [shorten(n, 30) for n in names]
        ...
        def export_summary_csv():
            ...
            try:
                with open(p, 'w', encoding='utf-8', newline='') as f:
                    ...
                    for name, cnt in win.pivot_data:
                        w.writerow([name, cnt])
        ...
        def update_view():
            """Apply sorting to current pivot data and refresh table + chart."""
            data = list(win.pivot_data)
            mode = win.sort_mode.get()
            if mode == 'name':
                data.sort(key=lambda kv: kv[0].lower())
            else:  # 'count_desc'
                data.sort(key=lambda kv: (-kv[1], kv[0].lower()))
            populate_table(data)
            draw_chart(data)
        ...
        def after_pivot_ready(data: List[Tuple[str, int]]):
            progress_lbl.configure(text=f"Summary ready. {len(data)} group(s).")
            win.pivot_data = data
            update_view()
