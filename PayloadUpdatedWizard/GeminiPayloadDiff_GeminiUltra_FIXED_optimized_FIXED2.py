...
class PayloadDiffViewerApp(tk.Tk):
    ...
    def on_view_summary(self):
        ...
        def update_view(*_):
            """Refresh table + chart + totals for the Summary window.
            This is safe to call even after the window is closed; it will
            simply return without touching destroyed widgets.
            """
            # If the window or labels are gone (user closed Summary), do nothing
            try:
                if not win.winfo_exists():
                    return
            except tk.TclError:
                return
            for w in (lbl_totals, status, tree):
                try:
        ...
        def after_pivot_ready(data: List[Tuple[str, int]]):
            """Called on the Tk main thread when the pivot worker completes.
            If the Summary window has been closed in the meantime, this
            safely does nothing.
            """
            try:
                if not win.winfo_exists():
