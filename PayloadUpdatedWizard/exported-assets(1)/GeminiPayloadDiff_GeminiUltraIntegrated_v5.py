...
class PayloadDiffViewerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Middle & bottom panes are added below
        ...
        self.txt_sel_old.tag_configure('del', background='#ffd6d6')
    ...
    def on_view_summary(self):
        ...
        totals_label = ttk.Label(chart_frame, text="")
        totals_label.pack(anchor='w', padx=6, pady=(0, 6))
        ...
        def apply_filters(data: List[Tuple[str, int]]) -> List[Tuple[str, int]]:
            """Apply selection, sort, search, and Top-N filters to pivot data."""
            # 1) Optional filter: only selected configs in the table
            try:
                sel = tree.selection()
                selected_names = [tree.item(iid, 'values')[0] for iid in sel]
            except Exception:
                selected_names = []

            if selected_names:
                selected_set = set(selected_names)
                data = [d for d in data if d[0] in selected_set]
            ...
            # 4) Top-N (only when nothing specifically selected)
            n = max(0, int(win.topn_var.get() or 0))
            if n > 0 and not selected_names:
                data = data[:n]
...
def draw_chart(data: List[Tuple[str, int]]):
        ...
        def update_view(*_):
            ...
            # Grand totals over all valid config names (before filters)
            total_configs = len(win.pivot_data)
            total_rows = sum(cnt for _, cnt in win.pivot_data) if win.pivot_data else 0
            totals_label.configure(
                text=f"Grand total: {total_configs} configs, {total_rows:,} rows"
            )

            status.configure(
                text=f"Summary ready. {total_configs} group(s). Showing {len(data)}."
            )
