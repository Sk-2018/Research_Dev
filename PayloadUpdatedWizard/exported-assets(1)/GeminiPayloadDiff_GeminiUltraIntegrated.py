...
# Optional external ultra-fast loader (separate module)
try:
...
def _load_csv_like_headers_rows_chunked(path: str, chunk_size: int = 20000,
    ...
    """
    CSV loader used by the UI for large files.

    Integration point:
    - First, try the external UltraFastLoader (if present).
    - If anything fails, fall back to the built-in pandas chunk reader.
    """
    # --- UltraFastLoader path (external module) ---
    if HAS_ULTRA_LOADER:
        try:
            ...
            total_rows = loader.estimate_rows(path, fmt="csv")
            if total_rows <= 0:
                total_rows = 1

            def ultra_cb(done, total):
                if progress_cb:
                    pct = int(done * 100 / max(1, total))
                    progress_cb(pct, f"Read ~{done:,} rows")

            for df in loader.load_in_chunks(path, chunk_size=chunk_size, progress_callback=ultra_cb):
                if not headers:
            ...
            logger.warning(f"UltraFastLoader failed, falling back to internal CSV loader: {e}")

    # --- Original chunked loader (pandas-based) ---
    if not HAS_PANDAS:
    ...
    try:
        ...
        logger.error(f"Pandas CSV load failed, falling back. Error: {e}")
        if progress_cb:
