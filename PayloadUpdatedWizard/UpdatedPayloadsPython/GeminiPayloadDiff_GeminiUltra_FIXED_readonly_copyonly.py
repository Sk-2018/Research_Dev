...
# --------------------------
# Helpers: Read-only Text widgets
# --------------------------

def make_text_readonly(widget: tk.Text) -> None:
    """
    Make a tk.Text widget behave as read-only for the user:
    - Disallow any edits (typing, backspace, delete, paste, cut).
    - Allow navigation, selection, Ctrl+C (copy), and Ctrl+A (select all).
    The widget itself stays in normal state so the program can still
    .delete() / .insert() as needed.
    """
    def _on_key(event):
        # Allow Ctrl+C (copy) and Ctrl+A (select all)
        if (event.state & 0x4) and event.keysym.lower() in ("c", "a"):
            return
        # Allow navigation keys (with or without Shift/Ctrl)
        if event.keysym in ("Left", "Right", "Up", "Down", "Home", "End", "Prior", "Next"):
            return
        # Block everything else (typing, backspace, delete, paste, cut, etc.)
        return "break"
...
class PayloadDiffViewerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        make_text_readonly(self.txt_sel_old)
        ...
        make_text_readonly(self.txt_sel_new)
        ...
        make_text_readonly(self.txt_old)
        make_text_readonly(self.txt_cur)
    ...
    def _make_text_readonly_copy(self, widget: tk.Text):
        """
        Make a Text widget behave like a read-only viewer: users cannot modify
        content but can select and copy text out to other tools.
        """
        ...
        # Right-click menu with only Copy / Select All
        ...
        # Keyboard shortcuts: select-all and copy are allowed
        widget.bind("<Control-a>", do_select_all)
        widget.bind("<Control-A>", do_select_all)
        widget.bind("<Control-c>", do_copy)
        widget.bind("<Control-C>", do_copy)

        # Block any attempt to modify the text (typing, delete, paste, cut)
        ...
        def on_key(event):
            ...
            if event.keysym in ("Left", "Right", "Up", "Down",
                                "Home", "End", "Prior", "Next"):
                return
            # Allow copy/select-all combinations (handled above)
