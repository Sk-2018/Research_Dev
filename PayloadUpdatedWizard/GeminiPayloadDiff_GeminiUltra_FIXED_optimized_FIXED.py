...
def parse_jsonish_verbose(s: str) -> Tuple[Any, str]:
    ...
    try:
        ...
        try:
            # Replace smart quotes and similar unicode punctuation that often
            # appear when payloads are copied from rich-text sources.
            t2 = t.replace('\u2018', "'").replace('\u2019', "'")
            t2 = t2.replace('\u201c', '"').replace('\u201d', '"')
            # Unescape any HTML entities (e.g. &quot;, &amp;)
            t2 = html.unescape(t2)

            # If the string contains a JSON-looking substring, attempt to
            # extract the first {...} or [...] block and parse that.
            first_brace = t2.find('{')
            first_bracket = t2.find('[')
            start = -1
            ...
            if start != -1:
                # Find the matching closing bracket by simple scanning.
                depth = 0
                end = -1
            ...
            # Heuristic: quote unquoted object keys like {a:1} -> {"a":1}
            try:
                quote_keys = re.sub(r'([\{,]\s*)([A-Za-z_]\w*)(\s*:)', r'\1"\2"\3', t2)
                if quote_keys != t2:
                    try:
                        return json.loads(quote_keys), ""
                    except Exception:
                        pass
            except Exception:
                pass

            # Fallback: try a lenient replacement of single quotes -> double quotes
            # only when no double quotes are present (common Python dict strings)
        ...
        # If the error came from ast parsing of a bare name like:  foo
        # ast raises a ValueError mentioning an <ast.Name object ...> which
        # is noisy; handle simple barewords by returning them as strings.
        try:
            emsg = str(e)
            if 'malformed node or string' in emsg or 'ast.Name' in emsg:
                # bareword pattern: letters/underscore then word chars
                if re.match(r'^[A-Za-z_]\w*$', t):
                    return t, ''
        except Exception:
            pass

        # Sanitize exception message to remove memory addresses like
        # '<ast.Name object at 0xABCDEF>' which leak implementation details
        try:
            raw = str(e)
            sanitized = re.sub(r"<([^>]+) object at 0x[0-9A-Fa-f]+>", r"<\1>", raw)
        except Exception:
            sanitized = str(e.__class__.__name__)

        return None, f"Failed to parse payload ({e.__class__.__name__}: {sanitized})"
...
ROLE_SYNONYMS = {
    ...
    "Config Key":  ["config key", "cfg key", "config_key", "cfg_key", "key", "identifier", "id", "cfgkey", "cfg-key", "cfgkey"],
...
def detect_best_columns(headers: List[str]) -> Tuple[Dict[str, int], Dict[str, float]]:
    ...
    # If the Config Key wasn't found, attempt a best-effort heuristic: look
    # for headers that resemble "cfgkey" or "config key" even if confidence
    # was slightly below threshold (handles compact headers like "CfgKey").
...
class PayloadDiffViewerApp(tk.Tk):
    ...
    def on_name_selected(self, _evt=None):
        ...
        keys = sorted(
            {
                self._format_key(self.rows[i].get('Config Key', ''))
                for i in indices
                if self.rows[i].get('Config Key', '').strip()
            }
        )
    ...
    def _get_rows_for_keys_map(self, name: str, keys: List[str]) -> Dict[str, Dict[str, str]]:
        """
        Build a map of {cfgkey: row_dict} for comparison using row indices.
        This ensures we only compare one row per key and keeps by_name compact.
        """
        key_set = set(keys)
        ...
        for idx in self.by_name.get(name, []):
            ...
            # Compare against the formatted representation shown in the UI
    ...
    def _diff_worker(self, tasks_q: queue.Queue, results_q: queue.Queue, parse_log_q: queue.Queue, ignore_order: bool):
        ...
        while True:
            task = tasks_q.get()
            if task is None:
                # Mark the sentinel task as done and exit
                tasks_q.task_done()
                break
            ...
            try:
                ...
                if err1:
                    parse_log_q.put((f"[{cfgkey}] OLD: {err1}", 'warning', old_str[:200]))
                if err2:
                    parse_log_q.put((f"[{cfgkey}] CURRENT: {err2}", 'warning', new_str[:200]))
                ...
                try:
                    ...
                    results_q.put((cfgkey, (old_obj, cur_obj), []))
                    continue
    ...
    def _format_key(self, k: str) -> str:
        ...
        k_str = str(k).strip()
        # Detect scientific notation more reliably (e.g. 1.23e+05 or -4E-03)
        try:
            if re.match(r'^[+-]?\d+(?:\.\d+)?[eE][+-]?\d+$', k_str):
                f = float(k_str)
                if f.is_integer():
                    return "{:.0f}".format(f)
                # return a non-exponential representation, trimming trailing zeros
                s = format(f, 'f')
