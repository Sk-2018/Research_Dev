...
def audit_write_csv(outdir: str, row: dict) -> str:
    ...
    fieldnames = ["timestamp","op","status","error_message","duration_ms","region","environment","host","port","db_name","schema","user","driver","mode","row_count","export_csv","export_xlsx","query_sha256","query_preview","row_breakdown"]
...
def connect_pg(host: str, port: int, dbname: str, user: str, pwd: str):
    options = f"-c statement_timeout={STATEMENT_TIMEOUT_MS} -c idle_in_transaction_session_timeout={IDLE_SESSION_TIMEOUT_MS} -c default_transaction_read_only=on -c application_name=payload_wizard"
    last = None
...
class PayloadComparisonFrame(ttk.Frame):
    def __init__(self, master, app):
        # Read-only SQL helper: load content then disable editing
        def set_sql_readonly(self, content: str):
        ...
        def copy_sql(self):
            sql_text = self.txt_sql.get("1.0","end-1c")
            self.clipboard_clear()
            self.clipboard_append(sql_text)
            self.log("SQL copied to clipboard.")
        ...
        self.txt_sql.configure(state="disabled")
        # define bound helpers so buttons/other methods can call them
        def _set_sql_readonly(content: str):
            self.txt_sql.configure(state="normal")
            self.txt_sql.delete("1.0", tk.END)
            self.txt_sql.insert("1.0", content)
            self.txt_sql.configure(state="disabled")
        self.set_sql_readonly = _set_sql_readonly
        ...
        self.btn_copy = ttk.Button(self, text="Copy SQL", command=self.copy_sql)
        self.btn_copy.grid(row=3, column=6, sticky="e", **pad)
    ...
    def load_defaults_from_selection(self, schema: str):
        self.set_sql_readonly(DEFAULT_SQL.format(schema=schema))
    ...
    def _worker(self, cfg, user, pwd, sql, outfile_csv: Path, outfile_xlsx: Path, use_copy: bool, outdir: str):
        ...
        conn = None; tmpcsv = None; rowcount = 0; result = "success"; errmsg = ""; row_breakdown = ""
        ...
        try:
            ...
            if use_copy:
                ...
                try:
                    if "Config Name" in df.columns:
                        _vc = df["Config Name"].value_counts(dropna=False)
                        row_breakdown = "; ".join([f"{k}:{int(v)}" for k, v in _vc.items()])
                    else:
                        row_breakdown = ""
                except Exception:
                    row_breakdown = ""
            ...
            if row_breakdown:
                self.log("Row breakdown (by Config Name):")
                for part in row_breakdown.split("; "):
                    self.log(f"  - {part}")
            ...
            preview = sanitize_sql_for_log(sql)
            qhash = sha256(preview.encode("utf-8","ignore")).hexdigest()
            ...
            audit_write_jsonl({
                ...
                "query_sha256": qhash, "query_preview": preview, "row_breakdown": row_breakdown, "connection_closed": True
            })
            ...
            try:
                csv_path = audit_write_csv(outdir, {
                    ...
                    "query_sha256": qhash, "query_preview": preview, "row_breakdown": row_breakdown,
                })
